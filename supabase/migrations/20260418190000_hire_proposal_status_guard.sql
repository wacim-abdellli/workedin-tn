-- Guard: prevent hiring proposals that were already declined/archived/withdrawn.
-- This keeps hire flow idempotent for accepted proposals with an existing contract
-- while blocking invalid status transitions.
...
COMMENT ON FUNCTION public.hire_proposal_atomic IS 'Atomically accepts a proposal, rejects competing proposals, creates a contract, and moves the job to in_progress. Guard added: only new/pending/shortlisted proposals are hireable.';
