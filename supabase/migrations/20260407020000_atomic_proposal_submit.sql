...
--   dynamically creates the function and applies grants/comment.
-- =============================================================================
...
BEGIN
    ...
    BEGIN
        ...
        -- 6. Deduct connects and log in the same transaction
    ...
    EXECUTE 'REVOKE ALL ON FUNCTION public.submit_proposal_atomic(UUID, TEXT, NUMERIC, INTEGER, JSONB, INTEGER) FROM PUBLIC';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.submit_proposal_atomic(UUID, TEXT, NUMERIC, INTEGER, JSONB, INTEGER) TO authenticated';
    EXECUTE $comment$
        COMMENT ON FUNCTION public.submit_proposal_atomic(UUID, TEXT, NUMERIC, INTEGER, JSONB, INTEGER)
        IS 'Atomically submits a proposal and deducts connects in a single transaction. Any failure at any step rolls back both operations. Enforces auth, open job, not own job, uniqueness, and connects balance. Freelancer profile completeness remains enforced by the JS canApplyToJob() pre-flight.';
    $comment$;
END;
$migration$;
