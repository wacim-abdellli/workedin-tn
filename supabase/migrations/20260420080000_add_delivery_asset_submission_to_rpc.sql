...
BEGIN
    ...
    SELECT
        ...
        'review_asset',
        'preview_available',
        COALESCE(asset->>'name', 'Review file'),
        COALESCE(asset->>'storage_bucket', 'contract-files'),
        COALESCE(asset->>'storage_path', ''),
    ...
    SELECT
        ...
        'final_asset',
        'locked',
        COALESCE(asset->>'name', 'Final file'),
        COALESCE(asset->>'storage_bucket', 'contract-files'),
        COALESCE(asset->>'storage_path', ''),
        NULLIF(asset->>'mime_type', ''),
        NULLIF(asset->>'size_bytes', '')::bigint,
    ...
    RETURN jsonb_build_object(
        ...
        'delivery_submitted_at', now(),
        'review_due_at', v_review_due_at,
        'review_asset_count', jsonb_array_length(COALESCE(p_review_assets, '[]'::jsonb)),
        'final_asset_count', jsonb_array_length(COALESCE(p_final_assets, '[]'::jsonb))
    );
END;
$$;
...
COMMENT ON FUNCTION public.submit_contract_delivery_atomic(uuid, text, jsonb, jsonb) IS 'Creates a formal versioned delivery with separate review assets and locked final assets, then moves the contract into delivery_submitted.';
