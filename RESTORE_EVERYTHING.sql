...
-- FREELANCER_PROFILES
CREATE POLICY IF NOT EXISTS "freelancer_profiles_select_all"
ON public.freelancer_profiles FOR SELECT
USING (true);

CREATE POLICY IF NOT EXISTS "freelancer_profiles_insert_own"
ON public.freelancer_profiles FOR INSERT
WITH CHECK (auth.uid() = id);

CREATE POLICY IF NOT EXISTS "freelancer_profiles_update_own"
ON public.freelancer_profiles FOR UPDATE
USING (auth.uid() = id);

-- JOBS - Everyone can view, clients can manage their own
CREATE POLICY IF NOT EXISTS "jobs_select_all"
...
CREATE POLICY IF NOT EXISTS "jobs_update_own"
ON public.jobs FOR UPDATE
USING (auth.uid() = client_id);

CREATE POLICY IF NOT EXISTS "jobs_delete_own"
ON public.jobs FOR DELETE
USING (auth.uid() = client_id);

-- PROPOSALS - Freelancers can manage their own
CREATE POLICY IF NOT EXISTS "proposals_select_involved"
...
CREATE POLICY IF NOT EXISTS "proposals_insert_own"
ON public.proposals FOR INSERT
WITH CHECK (auth.uid() = freelancer_id);

CREATE POLICY IF NOT EXISTS "proposals_update_own"
ON public.proposals FOR UPDATE
USING (auth.uid() = freelancer_id);

-- CONTRACTS - Involved parties can access
CREATE POLICY IF NOT EXISTS "contracts_select_involved"
ON public.contracts FOR SELECT
USING (auth.uid() = client_id OR auth.uid() = freelancer_id);

CREATE POLICY IF NOT EXISTS "contracts_insert_involved"
ON public.contracts FOR INSERT
WITH CHECK (auth.uid() = client_id OR auth.uid() = freelancer_id);

CREATE POLICY IF NOT EXISTS "contracts_update_involved"
ON public.contracts FOR UPDATE
USING (auth.uid() = client_id OR auth.uid() = freelancer_id);

-- MESSAGES - Involved parties can access
CREATE POLICY IF NOT EXISTS "messages_select_involved"
...
CREATE POLICY IF NOT EXISTS "portfolio_select_all"
...
CREATE POLICY IF NOT EXISTS "reviews_insert_involved"
ON public.reviews FOR INSERT
WITH CHECK (auth.uid() = reviewer_id);

-- IDENTITY_VERIFICATIONS - Users can see their own
CREATE POLICY IF NOT EXISTS "verifications_select_own"
ON public.identity_verifications FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "verifications_insert_own"
ON public.identity_verifications FOR INSERT
WITH CHECK (auth.uid() = user_id);

-- DISPUTES - Involved parties can access
CREATE POLICY IF NOT EXISTS "disputes_select_involved"
ON public.disputes FOR SELECT
USING (auth.uid() = opened_by);

CREATE POLICY IF NOT EXISTS "disputes_insert_own"
