...
function ProtectedRoute({ children }: { children: React.ReactNode }) {
  const { isAuthenticated, isFullyReady, profile, freelancerProfile } =
    useAuth();
  ...
  if (isFullyReady && profile && !isOnOnboardingPage) {
    ...
    const workspace =
      activeWorkspace || getInitialWorkspace(profile, freelancerProfile);

    // Check if onboarding is complete for the active workspace
    const onboardingComplete = isWorkspaceReady(
      profile,
      freelancerProfile,
      workspace,
    );
    ...
    if (!onboardingComplete) {
      ...
      return (
        <Navigate to={onboardingPath} replace state={{ from: location }} />
      );
    }
  }
