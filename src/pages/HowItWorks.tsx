            ...
            <section className="bg-white py-20 relative overflow-hidden">
                <div className="container-custom relative z-10 text-center">
                    <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-6">
                        {t.howItWorks.heroTitle} <span className="text-primary-600">{t.howItWorks.brandName}</span>
                    </h1>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {steps[activeTab].map((step, index) => (
                            <div key={index} className="card p-8 relative group hover:-translate-y-2 transition-transform duration-300">
                                <div className="absolute -top-6 right-8 w-12 h-12 rounded-2xl bg-white shadow-lg flex items-center justify-center text-xl font-bold text-gray-300 border border-gray-100 group-hover:text-primary-600 group-hover:border-primary-200 transition-colors">
                                    {index + 1}
                                </div>
                                <div className={`w-16 h-16 rounded-2xl mb-6 flex items-center justify-center ${activeTab === 'freelancer' ? 'bg-primary-50' : 'bg-secondary-50'
                                    }`}>
                                    {step.icon}
                                </div>
                    ...
                    <div className="space-y-4">
                        {t.howItWorks.faq.items.map((faq, index) => (
                            <div key={index} className="card overflow-hidden">
                                ...
                                {openFaq === index && (
                                    <div className="px-6 pb-6 text-muted leading-relaxed border-t border-gray-100 pt-4 bg-gray-50/50">
                                        {faq.a}
                                    </div>
                                )}
                            </div>
