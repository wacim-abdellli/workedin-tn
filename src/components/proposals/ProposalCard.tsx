...
function ProposalCard({ proposal, onMessage, onShortlist, onHire }: ProposalCardProps) {
    ...
    return (
        <div className={`bg-white rounded-2xl border transition-all duration-200 hover:shadow-md ${status === 'shortlisted' ? 'border-primary-200 ring-1 ring-primary-100' : 'border-gray-100'}`}>
            <div className="p-6">
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="flex-shrink-0 flex lg:flex-col gap-4 lg:w-48">
                        ...
                        <div>
                            <h3 className="font-bold text-gray-900 leading-tight hover:text-primary-600 cursor-pointer mb-1">
                                {freelancer.full_name}
                            </h3>
                            <p className="text-sm text-gray-500 mb-2">{freelancer.title || 'مستقل'}</p>
                    ...
                    <div className="flex-1 min-w-0 border-t lg:border-t-0 lg:border-r border-gray-100 pt-4 lg:pt-0 lg:pr-6">
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <span className="text-sm text-gray-500">
                                <strong className="text-gray-900">{freelancer.jobs_completed}</strong> وظيفة مكتملة
                            </span>
                            <span className="text-gray-300">|</span>
                            <span className="text-sm text-gray-500">
                                <strong className="text-gray-900">{freelancer.success_rate}%</strong> نجاح
                            </span>
                        </div>

                        <div className="relative">
                            <p className={`text-gray-600 leading-relaxed text-sm ${!expanded ? 'line-clamp-3' : ''}`}>
                                {cover_letter}
                            </p>
                                ...
                                {proposal.attachments.map((file: any, index: number) => (
                                    <div key={index} className="flex items-center gap-2 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg max-w-xs group cursor-pointer hover:bg-gray-100 transition-colors">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-medium text-gray-900 truncate">{file.name || `مرفق ${index + 1}`}</p>
                                            <p className="text-[10px] text-gray-500">{file.size || 'PDF'}</p>
                                        </div>
                    ...
                    <div className="lg:w-64 border-t lg:border-t-0 lg:border-r border-gray-100 pt-4 lg:pt-0 lg:pr-6 flex flex-col justify-between">
                        <div>
                            <div className="flex items-baseline justify-between mb-1">
                                ...
                                <span className="text-sm font-medium text-gray-500">د.ت</span>
                            </div>
                            ...
                            <div className="space-y-3 mb-6">
                                <div className="flex items-center justify-between text-sm">
                                    ...
                                    <span className="font-medium text-gray-900">{duration} يوم</span>
                                </div>
                                <div className="flex items-center justify-between text-sm">
                                    <span className="text-gray-500 flex items-center gap-1">
                                    ...
                                    <span className="font-medium text-gray-900">{bid_amount} د.ت</span>
                                </div>
                            </div>
                        </div>
