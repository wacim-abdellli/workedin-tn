...
export default function SearchModal({ onClose }: SearchModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 px-4" style={{ height: '52px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <input
            ...
            className="flex-1 bg-transparent text-gray-900 dark:text-white text-base outline-none placeholder:text-gray-400 dark:placeholder:text-gray-600"
          />
          {query && (
            <button onClick={() => setQuery('')} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" type="button">
              <X className="w-4 h-4" />
            </button>
          )}
          <kbd className="text-[11px] font-mono bg-gray-100 dark:bg-white/8 border border-gray-200 dark:border-white/10 rounded-md px-1.5 py-0.5 text-gray-400">
            ESC
          </kbd>
        </div>
          ...
          {!isSearching && (
            <div className="mb-2">
              <div className="flex items-center gap-1.5 px-2 py-1.5">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Trending now</span>
              </div>
              <div className="flex flex-wrap gap-1.5 px-2 pb-2">
                {TRENDING.map((tag) => (
                  <button
                    ...
                    className="text-xs font-medium px-2.5 py-1 rounded-lg transition-all duration-100"
                    style={{
          ...
          {items.map((item, idx) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-3 px-2 py-2.5 rounded-lg transition-colors duration-100 text-left"
                style={{
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.meta && <p className="text-xs text-gray-400 dark:text-gray-500">{item.meta}</p>}
                </div>
                {item.shortcut && (
                  <kbd className="text-[10px] font-mono text-gray-400 bg-gray-100 dark:bg-white/8 border border-gray-200 dark:border-white/10 rounded px-1.5 py-0.5 flex-shrink-0">
                    {item.shortcut}
                  </kbd>
                )}
              </button>
        ...
        >
          {[
            ...
            <div key={key} className="flex items-center gap-1.5">
              <kbd className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-gray-400">
                {key}
              </kbd>
