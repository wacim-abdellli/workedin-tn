    ...
    return (
        <div className="space-y-4 relative">
            ...
            <div className="grid gap-3 lg:grid-cols-3 relative z-10">
                <div className={`group relative overflow-hidden flex flex-col justify-between p-4 rounded-xl border backdrop-blur-xl shadow-xl transition-all duration-300 ${allEnabled ? 'border-brand/40 bg-brand/10 hover:border-brand/50' : 'border-white/10 bg-white/[0.02] hover:border-brand/30 hover:bg-brand/5'}`}>
                ...
                <div className="group relative overflow-hidden flex flex-col justify-between p-4 rounded-xl border border-white/10 bg-white/[0.02] backdrop-blur-xl shadow-xl transition-all duration-300 hover:border-purple-500/30 hover:bg-purple-500/5">
                    ...
                    {notifications.map((item) => {
                        ...
                        return (
                            <div 
                                ...
                                className={`group flex items-center justify-between gap-3 rounded-xl border p-5 transition-all duration-300 cursor-pointer ${item.enabled ? 'border-brand/40 bg-brand/5 shadow-[0_0_30px_-10px_rgba(var(--brand),0.2)]' : 'border-white/5 bg-white/[0.01] hover:border-white/20 hover:bg-white/[0.03]'}`}
                            >
                                <div className="flex min-w-0 items-center gap-3">    
                                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl shadow-inner transition-colors duration-500 ${item.enabled ? 'bg-brand/20 text-brand border border-brand/30' : 'bg-white/5 text-muted-foreground border border-white/10'}`}>
                                ...
                                <div className="shrink-0 flex items-center justify-center">
                                    <div className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors duration-500 shadow-inner ${item.enabled ? 'bg-brand shadow-[0_0_15px_rgba(var(--brand),0.6)]' : 'bg-black/40 border border-white/10'}`}>
                                        <span
                                            className={`inline-block h-6 w-6 transform rounded-full bg-white shadow-lg transition-transform duration-500 ease-in-out ${item.enabled ? 'translate-x-7' : 'translate-x-1'}`}
                                        />
                                    </div>
                                </div>
                            </div>
