...
export default function PaymentModal({ isOpen, onClose, amount, recipientName, onSuccess }: PaymentModalProps) {
    ...
    const renderD17 = () => (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="border-2 border-dashed border-border dark:border-dark-600 rounded-2xl p-8 text-center bg-surface dark:bg-dark-800/50 hover:bg-card dark:hover:bg-dark-800/80 hover:border-primary-200 dark:hover:border-primary-900 transition-all duration-300">
                <div className="w-48 h-48 bg-card p-2 rounded-xl shadow-sm border border-border mx-auto mb-4 flex items-center justify-center overflow-hidden">
                    ...
                    <div className="absolute bg-card p-2 rounded-full shadow-lg">
                        <Smartphone className="w-6 h-6 text-yellow-500" />
                    </div>
                </div>
                <p className="text-sm text-muted mb-3 font-medium">{t.payment.scanD17}</p>
                <div className="inline-flex items-center gap-3 px-4 py-2 bg-card dark:bg-dark-700 rounded-full border border-border dark:border-dark-600 shadow-sm">
            ...
            <div className="relative">
                <div className="absolute inset-0 flex items-center">
                    <span className="w-full border-t border-border dark:border-dark-700" />
                </div>
                <div className="relative flex justify-center text-xs uppercase font-medium">
                    <span className="bg-card dark:bg-dark-900 px-3 text-muted">{t.payment.orEnterPhone}</span>
                </div>
            </div>
    ...
    const renderFlouci = () => (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="text-center py-12 bg-surface dark:bg-dark-800/50 rounded-2xl border border-border dark:border-dark-700">
    ...
    const renderContent = () => {
        ...
        return (
            <div className="space-y-6">
                {/* Tabs */}
                <div className="flex bg-muted dark:bg-dark-800 p-1.5 rounded-xl border border-border dark:border-dark-700" role="tablist" aria-label={t.payment.chooseMethod}>
                    <button
                        type="button"
                        role="tab"
                        id="payment-tab-d17"
                        ...
                        className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 ${method === 'd17' ? 'bg-card dark:bg-dark-700 shadow text-primary-600 dark:text-primary-400' : 'text-muted hover:text-dark-900 dark:hover:text-white hover:bg-secondary'}`}
                    ...
                    <button
                        ...
                        className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 ${method === 'flouci' ? 'bg-card dark:bg-dark-700 shadow text-primary-600 dark:text-primary-400' : 'text-muted hover:text-dark-900 dark:hover:text-white hover:bg-secondary'}`}
                    ...
                    <button
                        ...
                        className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 ${method === 'card' ? 'bg-card dark:bg-dark-700 shadow text-primary-600 dark:text-primary-400' : 'text-muted hover:text-dark-900 dark:hover:text-white hover:bg-secondary'}`}
                ...
                {/* Footer */}
                <div className="flex items-center justify-between border-t border-border dark:border-dark-700 pt-6 mt-6">
                    <div>
