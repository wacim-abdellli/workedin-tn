...
export default function JobWizardLayout({
    ...
    return (
        <div className="mx-auto max-w-6xl space-y-5 lg:space-y-6">
            <section
                className="radius-shell overflow-hidden border p-6 sm:p-8 lg:p-9"
                style={{
                    borderColor: 'color-mix(in srgb, var(--workspace-primary) 18%, var(--border))',
                    background: 'radial-gradient(circle at top left, color-mix(in srgb, var(--workspace-primary) 14%, transparent), transparent 28%), radial-gradient(circle at top right, color-mix(in srgb, var(--brand-accent) 10%, transparent), transparent 24%), linear-gradient(145deg, color-mix(in srgb, var(--card-bg) 96%, white), color-mix(in srgb, var(--surface-bg) 94%, white))',
                    boxShadow: '0 36px 90px -52px color-mix(in srgb, var(--workspace-primary) 34%, transparent)',
                }}
            >
                <div className="grid gap-6 xl:grid-cols-[minmax(0,1.08fr)_320px]">
                    ...
                    <div className="rounded-[1.9rem] border p-5"
                        style={{
                ...
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    {steps.map((step) => {
                        ...
                        return (
                            <div
                                key={step.id}
                                className="rounded-[1.5rem] border p-4 transition-all duration-200"
                                style={isCurrent
                                    ? {
                                        ...
                                        boxShadow: '0 24px 48px -34px color-mix(in srgb, var(--workspace-primary) 46%, transparent)',
                                    }
                                    : isCompleted
                                        ? {
