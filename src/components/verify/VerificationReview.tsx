...
export default function VerificationReview({
    ...
    return (
        <div className="rounded-3xl border border-white/10 border-border bg-card/90 p-8 shadow-2xl shadow-slate-900/10 backdrop-blur-sm dark:border-white/10 border-border dark:bg-[var(--color-bg-elevated)]/90">
            <h2 className="text-2xl font-bold text-foreground dark:text-white mb-6 text-center">
                {tx('verifyIdentity.review.title', undefined, 'مراجعة البيانات')}
            </h2>

            {/* Readiness score */}
            <div className="mb-4 flex items-center justify-between rounded-xl border border-white/10 border-border bg-card/50 px-4 py-2 text-xs dark:bg-white/5">
                <span className="font-medium text-muted-foreground">{tx('verifyIdentity.review.readiness', undefined, 'Readiness score')}</span>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                    {[
                        ...
                        <div key={idx} className="space-y-2">
                            <p className="text-xs text-center font-medium text-muted">{item.title}</p>
                            <img src={item.src} alt={item.title} className="w-full h-24 object-cover rounded-lg border border-border" />
                        </div>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <button type="button" onClick={onEditFront} className="rounded-lg border border-border/80 px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-surface">
                        {tx('verifyIdentity.review.editFront', undefined, 'Edit front image')}
                    </button>
                    <button type="button" onClick={onEditBack} className="rounded-lg border border-border/80 px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-surface">
                        {tx('verifyIdentity.review.editBack', undefined, 'Edit back image')}
                    </button>
                    <button type="button" onClick={onEditSelfie} className="rounded-lg border border-border/80 px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-surface">
                ...
                {/* Consent checkbox */}
                <label htmlFor="identity-consent" className="flex items-center gap-3 p-4 border border-border rounded-xl hover:bg-surface transition-colors cursor-pointer">
                    <input
                        id="identity-consent"
                        type="checkbox"
                        checked={consent}
            ...
            <div className="flex items-center justify-between mt-8">
                <button onClick={onBack} className="flex items-center text-muted-foreground hover:text-foreground dark:hover:text-white transition-colors">
