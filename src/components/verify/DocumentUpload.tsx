...
export default function DocumentUpload({
    ...
    return (
        <div className="rounded-3xl border border-white/10 border-border bg-card/90 p-6 shadow-2xl shadow-slate-900/10 backdrop-blur-sm dark:border-white/10 border-border dark:bg-[var(--color-bg-elevated)]/90 md:p-8">
            ...
            <div className="mb-8">
                {preview ? (
                    <div className="relative overflow-hidden rounded-2xl border-2 border-primary-500/70 group">
                        ...
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <div className="flex items-center gap-3">
                                <button onClick={() => document.getElementById(inputId)?.click()} className="rounded-lg bg-white px-4 py-2 font-medium text-foreground transition-colors hover:bg-muted dark:text-white">
                                    {tx('verifyIdentity.changeImage', undefined, 'Change')}
                                </button>
                                <button onClick={onClear} className="inline-flex items-center gap-1 rounded-lg border border-white/40 bg-card/15 px-4 py-2 font-medium text-white transition-colors hover:bg-card/25">
                                    <X className="h-4 w-4" />
                    ...
                    <button
                        ...
                        className={`h-64 w-full cursor-pointer rounded-2xl border-2 border-dashed transition-colors group flex flex-col items-center justify-center ${dragging ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/15' : 'border-border hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/10'}`}
                    >
                        <div className="w-16 h-16 bg-muted rounded-full flex items-center justify-center text-muted group-hover:text-primary-500 mb-4 transition-colors">
                            <Upload className="w-8 h-8" />
                        </div>
                        <p className="text-muted-foreground font-medium">{tx('verifyIdentity.uploadHint', undefined, 'Click to upload image')}</p>
                        <p className="text-xs text-muted mt-1">{tx('verifyIdentity.dragDropHint', undefined, 'or drag and drop here')}</p>
                        <p className="text-sm text-muted mt-1">{tx('verifyIdentity.fileFormatHint', undefined, 'JPG, PNG (Max 5MB)')}</p>
                    </button>
                )}
