...
export default function InviteToJobModal({ isOpen, onClose, freelancerId, freelancerName }: InviteToJobModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={onClose}>
            <div 
                className="w-full max-w-lg bg-[#141414] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col"
            ...
            >
                ...
                <div className="p-5 overflow-y-auto max-h-[60vh]">
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-3.5 mb-6 bg-white/5 hover:bg-white/10 border border-dashed border-white/20 hover:border-white/40 rounded-xl text-white transition-all"
                    >
                    ...
                    <h3 className="text-xs font-bold tracking-wider text-white/40 uppercase mb-3">
                    ...
                    {isLoading ? (
                        ...
                        <div className="space-y-2">
                            {openJobs.map((job) => (
                                <div 
                                    key={job.id}
                                    className="flex items-center justify-between p-4 rounded-xl border border-white/10 bg-[#1a1a1a] hover:border-white/30 transition-colors group"
                                >
                                    <div className="flex-1 min-w-0 pr-4">
                                        <h4 className="text-sm font-semibold text-white truncate">{job.title}</h4>
                                        <p className="text-xs text-white/50 mt-1 capitalize">{job.job_type.replace('_', ' ')}</p>
                                    </div>
                                    <button
                                        onClick={() => handleInviteToJob(job)}
                                        disabled={isInviting === job.id}
                                        className="shrink-0 flex items-center gap-1.5 px-4 py-2 bg-[#8B5CF6] hover:bg-[#7c3aed] text-white text-sm font-semibold rounded-lg transition-colors disabled:opacity-50"
                                    >
                                        {isInviting === job.id ? (
